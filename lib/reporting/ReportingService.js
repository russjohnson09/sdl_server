...
// {
//   30ea6bce-91de-4b18-8b52-68d82112eee6: default
// }
   ...
   */
  async appUsageInsert(app_id,obj)
  {
    let {
    ...
    let result = await this.insert(`app_usage`,data);
  ...
  async updateAppLevelUsageAndErrorCounts(app_level,now,full_app_id_supported)
  {
    ...
    for (let uuid in app_level)
    {
      try {
        ...
        await this.appUsageInsert(application.id,app_level[uuid]);
      }
      catch (e)
      {
  ...
  async updatePolicyTableUpdateRequests(policyTableObject,now)
  {
    ...
    trigger_event = trigger_event || 'mileage';
  ...
  //http://policy.localhost/api/v1/applications/report?id=2
  ...
  async updateReporting(policyTableObject,now,full_app_id_supported)
  {
    ...
    try {
      ...
      // const full_app_id_supported = GET(req, "body.policy_table.module_config.full_app_id_supported", false) ? true : false;
      console.log(`full_app_id_supported`,full_app_id_supported)
...
;`,
  ...
  {
    console.log(`getAppUsageTimeHistory`,id);
    // usage_time_history: {2019-04-07: {minutes_in_hmi_full: 4, minutes_in_hmi_background: 6, minutes_in_hmi_limited: 6,…},…}
    // 2019-04-07: {minutes_in_hmi_full: 4, minutes_in_hmi_background: 6, minutes_in_hmi_limited: 6,…}
    // minutes_in_hmi_background: 6
    // minutes_in_hmi_full: 4
    // minutes_in_hmi_limited: 6
    // minutes_in_hmi_none: 8
...
;`,
    ...
    console.log(`getAppUsageTimeHistory`,rows);
    ...
    for (let row of rows)
    {
      ...
      for (let hmi_key in ['minutes_in_hmi_background','minutes_in_hmi_full','minutes_in_hmi_limited','minutes_in_hmi_none'])
      {
  ...
  async getAppUserSelectionHistory()
  {
    // user_selection_history: {2019-04-06: {count_of_user_selections: 7}, 2019-04-07: {count_of_user_selections: 1},…}
    // 2019-04-06: {count_of_user_selections: 7}
    // count_of_user_selections: 7
  }

  async getAppRejectedRPCsHistory()
  {
    // rejected_rpcs_history: {2019-04-06: {count_of_rejected_rpcs_calls: 8}, 2019-04-07: {count_of_rejected_rpcs_calls: 8},…}
    // 2019-04-06: {count_of_rejected_rpcs_calls: 8}
    // count_of_rejected_rpcs_calls: 8
  }
  ...
  async getAppUsageReport(appId)
  {
    ...
    try {
      ...
      // let rejected_rpcs_history = await this.getAppRejectedRPCsHistory();
      // let user_selection_history = await this.getAppUserSelectionHistory();
