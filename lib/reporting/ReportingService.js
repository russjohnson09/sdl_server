...
class ReportingService
{
  ...
  async getTotalDeivceModels(expireDate)
  {
    expireDate = expireDate || this.getExpireDate();
    return this.doQuery(
      {
        text: `select count(id),hardware from device where updated_ts > $1 group by hardware`,
        values: [expireDate]
      });
  }
  ...
  async getTotalDeviceCarrier(expireDate)
  {
    ...
    return this.doQuery(
      {
        text: `select count(id),carrier from device where updated_ts > $1 group by carrier`,
        values: [expireDate]
      });
  }
  ...
  async getTotalDeviceOs(expireDate)
  {
    ...
    return this.doQuery(
      {
        text: `select count(id),os from device where updated_ts > $1 group by os`,
        values: [expireDate]
      });
  }
  ...
  async getDeviceReport()
  {
    ...
    let total_device_model = await this.getTotalDeivceModels(expireDate);
