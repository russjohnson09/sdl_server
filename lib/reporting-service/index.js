...
//     },
    ...
    insert:  function(table,data,now)
    {
        ...
        let text = `INSERT1 INTO ${table}(${columns.join(',')}) VALUES (${value_params.join(',')})`;
    ...
    insertUpdateRecordById: function(table,data,now)
    {
        ...
        return reportingService.sqlCommand(query);
    },
    //TODO use callback instead
...
;`,
    ...
    getAppUsageTimeHistory: async function(id)
    {
        ...
        let {rows} = await reportingService.sqlCommand(slqCommand);
    ...
    sqlCommand: async function(query)
    {
        let longQueryTime = 1;
        let t1 = Date.now();
